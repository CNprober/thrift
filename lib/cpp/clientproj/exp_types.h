...
class Location {
 public:
  ...
  double latitude;  //required double latitude;
  double longitude; //required double longitude;
  ...
  bool operator < (const Location & ) const;    //未实现,不能被调用
...
class Tweet {
 public:
  ...
  int32_t userId;               //required i32 userId;
  std::string userName;         //required string userName;
  std::string text;             //required string text;
  Location loc;                 //optional Location loc;
  TweetType::type tweetType;    //optional TweetType tweetType = TweetType.TWEET;
  std::string language;         //optional string language = "english";
